...
function Show-Help {
    Write-ColorOutput "瓜呱图文社区 Docker 部署脚本" "Cyan"
...
function Seed-Data {
    ...
    $mysqlHealthy = Wait-ContainerHealthy -ContainerName 'guagua-mysql' -TimeoutSeconds 180
    $backendHealthy = Wait-ContainerHealthy -ContainerName 'guagua-backend' -TimeoutSeconds 180
    ...
    docker-compose -p guagua exec -T backend node scripts/generate-data.js
...
function Start-Services {
    Write-ColorOutput "启动瓜呱图文社区服务..." "Green"

    if ($Build) {
        Write-ColorOutput "重新构建镜像..." "Yellow"
        docker-compose -p guagua down
        docker-compose -p guagua build --no-cache
    }

    docker-compose -p guagua up -d
    if ($LASTEXITCODE -eq 0) {
...
function Clean-Resources {
    ...
    if ($confirmation -match '^[Yy]$') {
        ...
        docker-compose -p guagua down -v --rmi all
...
function Show-Logs {
    ...
    docker-compose -p guagua logs -f
}

function Show-Status {
    Write-ColorOutput "服务状态:" "Cyan"
    docker-compose -p guagua ps
}
