      ...
      <div class="sidebar" :class="{ collapsed: isCollapsed, expanded: isExpanded }" @mouseenter="handleMouseEnter"
        ...
        <div class="sidebar-header">
          <div class="logo">
            ...
            <h2 class="logo-text">瓜呱管理后台</h2>
          </div>
        </div>
        <nav class="sidebar-nav">
          <div v-for="grp in allowedGroups" :key="grp.group" class="nav-group">
            <div class="nav-group-title" v-if="!isCollapsed || isExpanded">{{ grp.group }}</div>
            <router-link v-for="item in grp.items" :key="item.path" :to="item.path" class="nav-item"
        ...
        <div class="sidebar-footer">
          <div v-if="isExpanded || !isCollapsed" class="admin-info-block">
            <div class="admin-name">{{ adminStore.admin?.username || '管理员' }}</div>
            <div class="admin-role-badge">{{ currentRoleLabel }}</div>
          </div>
          ...
          <DropdownMenu class="mobile-only" direction="down" menu-class="mobile-admin-menu">
            ...
            <template #menu>
              <div class="mobile-menu-content">
                <template v-for="grp in allowedGroups" :key="grp.group">
                  <div class="mobile-menu-group">{{ grp.group }}</div>
                  <DropdownItem v-for="item in grp.items" :key="item.path" @click="() => $router.push(item.path)">
...
import { getAllowedGroups, getRoleLabel, getTitleByPath } from './permissions'
...
const shouldShowFilter = computed(() => {
  const noFilterRoutes = ['/admin/api-docs', '/admin/monitor', '/admin/shop-categories', '/admin/products', '/admin/orders', '/admin/ai-analysis', '/admin/behavior', '/admin/trace', '/admin/knowledge']
  if (noFilterRoutes.includes(route.path)) return false
  if (route.path.startsWith('/admin/products/')) return false
  return true
})
...
const allowedGroups = computed(() => getAllowedGroups(adminStore.role))
const currentRoleLabel = computed(() => getRoleLabel(adminStore.role))

const currentPageTitle = computed(() => {
  if (route.path.startsWith('/admin/products/edit')) return '编辑商品'
  if (route.path.startsWith('/admin/products/create')) return '新增商品'
  return getTitleByPath(route.path)
})

const currentPageDescription = computed(() => {
  const descriptions = {
    '/admin/ai-analysis': 'AI 智能经营分析，基于订单与商品数据生成洞察与优化建议',
    '/admin/behavior': '用户行为分析',
    '/admin/trace': '查看 AI 问答全链路 Trace，定位检索/路由/模型各步骤耗时与异常',
    '/admin/knowledge': '管理 AI 知识库：FAQ 增删改查、文档增量入库、全量重建',
    ...
    '/admin/shop-categories': '管理商城商品分类，支持层级与启用状态',
    '/admin/products': '管理商城商品，支持上下架、库存调整与编辑',
    '/admin/orders': '管理商城订单，支持发货、关闭与详情查看'
  }
...
.admin-role-badge {
  ...
  padding: 2px 8px;
  border-radius: 4px;
  background: var(--nav-active-bg, rgba(220, 18, 87, 0.1));
...
.sidebar.collapsed.expanded .nav-group-title {
  opacity: 1;
  height: auto;
  padding: 14px 16px 6px 16px;
}
...
.nav-item.active {
  background-color: var(--nav-active-bg, rgba(220, 18, 87, 0.08));
  color: var(--primary-color);
  font-weight: 600;
}
