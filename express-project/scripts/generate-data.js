...
const { uploadImageToOSS } = require('../utils/ossUploader');


// 模拟数据生成器
class DataGenerator {
  constructor() {
    // OSS 上传缓存：避免同一张网络图片重复下载/上传
    this.ossUrlCache = new Map();
    // 上传失败计数，用于兜底
    this.ossUploadFailCount = 0;
    // 使用中文分类名称
    this.categories = [
      '学习', '校园', '情感', '兴趣',
      '生活', '社交', '求助', '观点', '毕业', '职场'
  ...
  //生成随机图片URL（用于笔记图片）
  // 生成基于 seed 的随机占位图 URL（使用 picsum.photos，无版权）
  generateRandomImageUrl() {
    const seed = Math.random().toString(36).substring(2, 10);
    const width = 400 + Math.floor(Math.random() * 200); // 400-600
    const height = 500 + Math.floor(Math.random() * 400); // 500-900
  ...
  /**
   * 从网上下载图片并上传到阿里云 OSS
   * @param {string} imageUrl - 网络图片 URL
   * @returns {Promise<string>} OSS 访问 URL（失败则返回原 URL）
   */
  async uploadImageToOSSFromUrl(imageUrl) {
    if (!imageUrl) return imageUrl;

    // 命中缓存则直接返回
    if (this.ossUrlCache.has(imageUrl)) {
      return this.ossUrlCache.get(imageUrl);
    }
    ...
    try {
      ...
      const filename = `seed_${Date.now()}_${Math.random().toString(36).substring(2, 8)}${ext}`;

      const result = await uploadImageToOSS(buffer, filename, contentType);

      if (result.success && result.url) {
        this.ossUrlCache.set(imageUrl, result.url);
        return result.url;
      } else {
        throw new Error(result.message || 'OSS 上传失败');
      }
    } catch (error) {
      this.ossUploadFailCount++;
      console.warn(`图片上传失败，使用原链接: ${imageUrl}, 错误: ${error.message}`);
      this.ossUrlCache.set(imageUrl, imageUrl); // 缓存原链接，避免重复失败
      return imageUrl;
    }
  }
  ...
  /**
   * 批量并发上传网络图片到 OSS
   * @param {string[]} imageUrls - 网络图片 URL 数组
   * @param {string} label - 日志标签（如"头像"、"笔记图片"）
   * @param {number} batchSize - 并发批次大小
   * @returns {Promise<Map<string, string>>} URL -> OSS URL 映射
   */
  async batchUploadImagesToOSS(imageUrls, label = '图片', batchSize = 8) {
    const uniqueUrls = [...new Set(imageUrls.filter(url => url))];
    console.log(`  开始批量上传 ${uniqueUrls.length} 张${label}到 OSS（并发数: ${batchSize}）...`);
    ...
    for (let i = 0; i < uniqueUrls.length; i += batchSize) {
      ...
      const batchResults = await Promise.all(
        batch.map(async (url) => {
          const ossUrl = await this.uploadImageToOSSFromUrl(url);
          return { url, ossUrl };
        })
      );

      for (const { url, ossUrl } of batchResults) {
        results.set(url, ossUrl);
      }

      completed += batch.length;
      console.log(`  ${label}上传进度: ${completed}/${uniqueUrls.length}`);
      await this.delay(100); // 短暂间隔，避免触发 OSS 限流
    }

    console.log(`  ✅ ${label}批量上传完成，成功 ${uniqueUrls.length - this.ossUploadFailCount} 张`);
    return results;
  }
    ...
    for (const table of tables) {
      try {
        ...
        if (!error.message.includes("doesn't exist")) {
          console.warn(`清空 ${table} 表失败: ${error.message}`);
        }
      }
    }
  ...
  async insertUsers(connection, users) {
    // 先批量并发上传所有头像到 OSS
    const avatarUrls = users.map(user => user.avatar);
    const avatarUrlMap = await this.batchUploadImagesToOSS(avatarUrls, '头像', 8);

    for (const user of users) {
      const avatarUrl = avatarUrlMap.get(user.avatar) || user.avatar;
      const result = await connection.execute(
        ...
        [user.user_id, user.password, user.nickname, avatarUrl, user.bio, user.location, 0, 0, 0, user.is_active, user.last_login_at, user.gender, user.zodiac_sign, user.mbti, user.education, user.major, user.interests]
      );
      // 更新用户对象的id字段为数据库自增id
  ...
  async insertPostImages(connection, postImages) {
    ...
    const imageUrlMap = await this.batchUploadImagesToOSS(imageUrls, '笔记图片', 8);

    for (const image of postImages) {
      const imageUrl = imageUrlMap.get(image.image_url) || image.image_url;
      await connection.execute(
        'INSERT INTO post_images (post_id, image_url) VALUES (?, ?)',
        [image.post_id, imageUrl]
      );
    }
  ...
  generateUsers(count = 50) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const user = {
        user_id: `user${String(i + 1).padStart(3, '0')}`, // 瓜呱号，字符串格式
  ...
  async insertData() {
    ...
    try {
      ...
      if (this.ossUploadFailCount > 0) {
        console.warn(`   OSS 图片上传失败: ${this.ossUploadFailCount} 张（已回退使用原网络链接）`);
      }
