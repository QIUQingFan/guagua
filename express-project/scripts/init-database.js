...
class DatabaseInitializer {
  ...
  async initializeTables() {
    ...
    try {
      ...
      // 兼容旧库：补齐 admin.role 列
      await this.alterAdminAddRoleColumn(connection);
      ...
      // 创建私聊消息表
      await this.createPrivateMessagesTable(connection);

      // 创建会话列表表
      await this.createChatSessionsTable(connection);

      // 创建群聊表
      await this.createChatGroupsTable(connection);

      // 创建群成员表
      await this.createGroupMembersTable(connection);

      // 创建群消息表
      await this.createGroupMessagesTable(connection);
  ...
  async createUsersTable(connection) {
    const sql = `
      CREATE TABLE IF NOT EXISTS \`users\` (
        ...
        \`user_id\` varchar(50) NOT NULL COMMENT '瓜呱号',
  ...
  async createAdminTable(connection) {
    const sql = `
      CREATE TABLE IF NOT EXISTS \`admin\` (
        ...
        \`role\` varchar(20) NOT NULL DEFAULT 'super_admin' COMMENT '角色: super_admin-超级管理员, developer-开发人员, merchant-商家',
        \`created_at\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
        PRIMARY KEY (\`id\`),
  ...
  // 兼容已存在的库：若 admin 表缺少 role 列则补齐
  async alterAdminAddRoleColumn(connection) {
    try {
      const [rows] = await connection.execute(
        `SELECT COUNT(*) AS cnt FROM information_schema.COLUMNS
         WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'admin' AND COLUMN_NAME = 'role'`
      );
      const hasRole = rows && rows[0] && Number(rows[0].cnt) > 0;
      if (!hasRole) {
        await connection.execute(
          `ALTER TABLE \`admin\` ADD COLUMN \`role\` varchar(20) NOT NULL DEFAULT 'super_admin' COMMENT '角色: super_admin-超级管理员, developer-开发人员, merchant-商家'`
        );
        console.log('✓ admin 表新增 role 列');
      }
    } catch (error) {
      console.error('检查/新增 admin.role 列失败:', error.message);
    }
  }
  ...
  async createPrivateMessagesTable(connection) {
    const sql = `
      CREATE TABLE IF NOT EXISTS \`private_messages\` (
        ...
        \`type\` tinyint(4) DEFAULT 1 COMMENT '消息类型: 1-文本, 2-图片, 3-表情',
        \`is_read\` tinyint(1) DEFAULT 0 COMMENT '是否已读',
        ...
        CONSTRAINT \`pm_ibfk_1\` FOREIGN KEY (\`from_user_id\`) REFERENCES \`users\` (\`id\`) ON DELETE CASCADE,
        CONSTRAINT \`pm_ibfk_2\` FOREIGN KEY (\`to_user_id\`) REFERENCES \`users\` (\`id\`) ON DELETE CASCADE
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='私聊消息表';
    `;
    await connection.execute(sql);
    console.log('✓ private_messages 表创建成功');
  }

  async createChatSessionsTable(connection) {
    const sql = `
      CREATE TABLE IF NOT EXISTS \`chat_sessions\` (
        \`id\` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '会话ID',
        \`user_id\` bigint(20) NOT NULL COMMENT '用户ID',
        \`session_type\` tinyint(4) NOT NULL COMMENT '会话类型: 1-私聊, 2-群聊',
        \`target_id\` bigint(20) NOT NULL COMMENT '目标ID(用户ID或群ID)',
  ...
  async createChatGroupsTable(connection) {
    const sql = `
      CREATE TABLE IF NOT EXISTS \`chat_groups\` (
        ...
        KEY \`idx_created_at\` (\`created_at\`),
        CONSTRAINT \`cg_ibfk_1\` FOREIGN KEY (\`owner_id\`) REFERENCES \`users\` (\`id\`) ON DELETE CASCADE
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='群聊表';
    `;
    await connection.execute(sql);
    console.log('✓ chat_groups 表创建成功');
  }

  async createGroupMembersTable(connection) {
    const sql = `
      CREATE TABLE IF NOT EXISTS \`group_members\` (
        \`id\` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '成员ID',
        \`group_id\` bigint(20) NOT NULL COMMENT '群ID',
        \`user_id\` bigint(20) NOT NULL COMMENT '用户ID',
        \`role\` tinyint(4) DEFAULT 2 COMMENT '角色: 1-群主, 2-成员',
        \`joined_at\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '加入时间',
        PRIMARY KEY (\`id\`),
        UNIQUE KEY \`uk_group_user\` (\`group_id\`, \`user_id\`),
        KEY \`idx_group_id\` (\`group_id\`),
        KEY \`idx_user_id\` (\`user_id\`),
        CONSTRAINT \`gm_ibfk_1\` FOREIGN KEY (\`group_id\`) REFERENCES \`chat_groups\` (\`id\`) ON DELETE CASCADE,
        CONSTRAINT \`gm_ibfk_2\` FOREIGN KEY (\`user_id\`) REFERENCES \`users\` (\`id\`) ON DELETE CASCADE
  ...
  async createGroupMessagesTable(connection) {
    const sql = `
      CREATE TABLE IF NOT EXISTS \`group_messages\` (
        ...
        KEY \`idx_from_user_id\` (\`from_user_id\`),
        KEY \`idx_created_at\` (\`created_at\`),
        CONSTRAINT \`gmsg_ibfk_1\` FOREIGN KEY (\`group_id\`) REFERENCES \`chat_groups\` (\`id\`) ON DELETE CASCADE,
        CONSTRAINT \`gmsg_ibfk_2\` FOREIGN KEY (\`from_user_id\`) REFERENCES \`users\` (\`id\`) ON DELETE CASCADE
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='群消息表';
    `;
    await connection.execute(sql);
    console.log('✓ group_messages 表创建成功');
  }
  ...
  async insertDefaultAdmin() {
    ...
    try {
      ...
      // 默认管理员账户（密码均为 123456 的 SHA-256）
      // - admin    → super_admin  超级管理员（全量权限）
      // - dev01    → developer    开发人员（仅 AI 监控类数据）
      // - shop01   → merchant     商家（仅店铺运营类数据）
      const pwdHash = '8d969eef6ecad3c29a3a629280e686cf0c3f5d5a86aff3ca12020c923adc6c92';
      const adminSql = `
        INSERT INTO \`admin\` (\`username\`, \`password\`, \`role\`) VALUES
          ('admin',  '${pwdHash}', 'super_admin'),
          ('dev01',  '${pwdHash}', 'developer'),
          ('shop01', '${pwdHash}', 'merchant')
